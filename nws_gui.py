...
class WeatherGUI:
    ...
    def fetch_weather(self, *args):
        try:
            ...
            self.dewpoint_var.set(
                f"{self.weather.dewpoint}°F"
                if hasattr(self.weather, "dewpoint")
                else "N/A"
            )
            self.windchill_var.set(
                f"{self.weather.windchill}°F"
                if hasattr(self.weather, "windchill")
                else "N/A"
            )
            self.heatindex_var.set(
                f"{self.weather.heatindex}°F"
                if hasattr(self.weather, "heatindex")
                else "N/A"
            )
            self.elevation_var.set(
                f"{self.weather.elevation} ft"
                if hasattr(self.weather, "elevation")
                else "N/A"
            )
            ...
            if hasattr(self.weather, "forecast_hourly_list"):
                for index, period in enumerate(
                    self.weather.forecast_hourly_list[:24]
                ):
            ...
            if hasattr(self.weather, "forecast_list"):
                for index, period in enumerate(self.weather.forecast_list):
                    ...
                    self.detailed_forecast_tree.insert(
                        "",
                        "end",
                        values=(
                            period["name"],
    ...
    def create_widgets(self):
        ...
        self.notebook.add(
            self.detailed_forecast_frame, text="7-Day Detailed Forecast"
        )
        ...
        alerts_scrollbar = ttk.Scrollbar(
            self.alerts_frame, orient="vertical", command=self.alerts_text.yview
        )
        ...
        self.coords_frame.grid(row=1, column=0, columnspan=2, sticky="we")
        ...
        ttk.Label(
            self.coords_frame, textvariable=self.address_var, wraplength=900
        ...
        self.location_frame.grid(row=2, column=0, columnspan=2, sticky="we")
        ...
        self.hourly_tree = ttk.Treeview(
            ...
            columns=("Time", "Temp", "Wind Spd", "Wind Dir", "Forecast"),
            show="headings",
            height=TREE_HEIGHT,
        )
        ...
        self.detailed_forecast_tree = ttk.Treeview(
            ...
            columns=("Period", "Details"),
            show="headings",
            height=TREE_HEIGHT,
        )

        # Define alternating row tags for banded row display
        self.detailed_forecast_tree.tag_configure("oddrow", background="gray30")
        ...
        xdetailed_forecast_scrollbar.grid(row=1, column=0, sticky="ew")
        ...
        detailed_forecast_scrollbar.grid(row=0, column=1, sticky="ns")
