...
# from ttkbootstrap.ttk import Frame, Scrollbar, Treeview, Notebook
...
# ----------------------- CREATE WIDGETS --------------------------------- #
    def create_widgets(self):
        ...
        self.location_frame = ttk.LabelFrame(self.root, text="Location", padding="5")
        self.location_frame.grid(row=1, column=0, columnspan=2, sticky=(W, E), pady=5, padx=5)

        # City/State Entry
        ttk.Label(self.location_frame, text="City:").grid(row=0, column=0, padx=5)
        self.city_entry = ttk.Entry(self.location_frame, width=30)
        self.city_entry.grid(row=0, column=1, padx=5)

        ttk.Label(self.location_frame, text="State:").grid(row=0, column=2, padx=5)
        ...
        self.get_weather_btn = ttk.Button(self.location_frame, text="Get Weather", command=self.fetch_weather)
        self.get_weather_btn.grid(row=0, column=4, padx=10)

        # Coordinates Display Frame
        self.coords_frame = ttk.LabelFrame(self.root, text="Coordinates", padding="5")
        ...
        ttk.Label(self.coords_frame, textvariable=self.address_var, wraplength=900).grid(row=1, column=0, padx=5)
        ttk.Label(self.coords_frame, textvariable=self.coords_var).grid(row=2, column=0, columnspan=2, padx=5, sticky=W)
        ttk.Label(self.coords_frame, textvariable=self.station_var).grid(row=3, column=0, columnspan=2, padx=5, sticky=W)
        ...
        for i, (label, var) in enumerate(weather_items):
            ttk.Label(self.current_weather_frame, text=label).grid(row=i, column=0, sticky=W, padx=5)
            ttk.Label(self.current_weather_frame, textvariable=var).grid(row=i, column=1, sticky=W)

        # Alerts Frame
        self.alerts_text = ttk.Text(self.alerts_frame, width=100, height=14)
        self.alerts_text.grid(row=0, column=0, sticky=(W, E, N, S))
        # Add vertical scrollbar to alerts frame
        alerts_scrollbar = ttk.Scrollbar(self.alerts_frame, orient="vertical", command=self.alerts_text.yview)
        ...
        self.hourly_tree = ttk.Treeview(self.hourly_forecast_frame, columns=("Time", "Temp", "Wind", "Forecast"), show="headings")
        ...
        hourly_scrollbar = ttk.Scrollbar(self.hourly_forecast_frame, orient="vertical", command=self.hourly_tree.yview)
        ...
        self.forecast_tree = ttk.Treeview(self.forecast_frame, columns=("Period", "Temperature", "Wind", "Forecast"), show="headings")
        ...
        forecast_scrollbar = ttk.Scrollbar(self.forecast_frame, orient="vertical", command=self.forecast_tree.yview)
        ...
        self.detailed_forecast_tree = ttk.Treeview(self.detailed_forecast_frame, columns=("Period", "Details"), show="headings")
        ...
        detailed_forecast_scrollbar = ttk.Scrollbar(self.detailed_forecast_frame, orient="vertical", command=self.detailed_forecast_tree.yview)
    ...
    def fetch_weather(self, *args):
        try:
            ...
            if not city or not state:
                ttk.messagebox.showerror("Error", "Please enter both city and state")
                return
